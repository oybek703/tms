...
const DashboardCurrencyRates = ({cbRate = []}) => {
    return (
        <Table size='small'>
            <TableHead>
                <TableRow>
            ...
            <TableBody>
                {currencyOrder.map(currName => {
                    const row = cbRate.find(curr => curr['nominal'] === `1 ${currName}`)
...
const ExternalCurrencyRates = ({rates = []}) => {
    ...
    return (
        <Table size='small' style={{height: '100%'}}>
            ...
            <TableBody>
                {currencyOrder.map(currName => {
                    const row = rates.find(curr => curr['nominal'] === `1 ${currName}`) || {}
                    return <TableRow key={uuid()}>
                        <TableCell align='center'>{(row || {})['nominal']}</TableCell>
...
export default DashboardCurrencyRates
export {ExternalCurrencyRates}