...
function NoBorderCell(props) {
    ...
    `} align='center' children={children} {...props}/>
}

function RedLightText(props) {
    const classes = useStyles()
    const {children} = props
    return <b className={classes.lightRed} children={children} {...props}/>
}

function FcrbTable({rows = {}}) {
    const classes = useStyles()
    let {mfiData = {}, treasuryData = {}, retailData = {}, centralizedResourceBaseData = {}, portfolioData = {}} = rows
    return (
        <Paper>
            <Table size='small' stickyHeader children={<TableCap rows={20} text={'млрд.сум'}/>}/>
            ...
            <Grid container className={classes.smallCardContainer}>
                                ...
                                {['USD', 'EUR', 'UZS'].map((t, i) => <Fragment key={uuid()}>
                                    <NoBorderCell colSpan={2} morered='true'
                                        ...
                                        <b>{t} &nbsp; <RedLightText>
                                            {formatNumber(
                                                ((mfiData['mfiPercents'] || [])
                                                    .find(percent => percent['CURRENCY_NAME'] === t) || {})['PERCENT']
                                            )}%
                                        </RedLightText></b>
                                    </NoBorderCell>
                                </Fragment>)}
                ...
                <Grid className={classes.smallCard} item xs={4}>
                    <Table className={classes.borderRadius} size='small'>
                        ...
                        <TableBody>
                            <TableRow children={<NoBorderCell colSpan={6} redbold='true' titlenumber='true' children={
            ...
            <Grid container justify='center' className={classes.resourceBase}>
            ...
            {/* БАЛАНСОВЫЕ АКТИВЫ */}
            <br/>
            <Grid container className={`${classes.smallCardContainer} ${classes.balanceActive}`} justify='center'>
                <Grid item xs={12}>
                    <Table className={classes.borderRadius} size='small'>
                        <TableHead>
                            <TableRow>
            ...
            <Grid container className={`${classes.smallCardContainer} ${classes.fundingAvg}`} justify='space-evenly'>
                <Grid item xs={6}>
                    <Table className={classes.borderRadius} size='small'>
                        <TableHead>
                            <TableRow>
