...
import CorrespondentTab from '../UI/Layout/Tabs/CorrespondentTab'
import LimitOfBanks from './LimitOfBanks'
...
function CorrespondentTable({rows, currentState = false}) {
    const {currencyRate = [], totalCash = [], interbankDeposits = [], limitOfBanks = {}} = rows
    const [expanded, setExpanded] = useState('main')
    const classes = useStyles()
    const {reportDate} = useSelector(state => state.date)
    const handleTabChange = useCallback(code => setExpanded(code), [])
    const correspondentTableRendered = (<TableContainer component={Paper}>
        <ExportButton id={`correspondent-${currentState ? 'realtime' : formatOneDate(reportDate)}`}/>
        <Table tableexport-key={`correspondent-${currentState ? 'realtime' : formatOneDate(reportDate)}`}
            ...
            <TableBody>
                {currencyRate.map((row, i) => (
                    <StyledTableRow key={`${row.state}+${i}`}>
                        ...
                        <TableCell className={classes.noWrap} align="center">{row['isTableHead'] ?
                            <b>{formatNumber(row.UZS, 'e')}</b> : formatNumber(row.UZS, 'e')}</TableCell>
                        {i !== 1
                            ? <>
    ...
    return (
        <>
            {currentState ? (<>
                <CorrespondentTab active={expanded} handleChange={handleTabChange}/>
                {expanded === 'main' && correspondentTableRendered}
                {expanded === 'limit' && <LimitOfBanks rows={limitOfBanks}/>}
            </>) : correspondentTableRendered}
        </>
    )
}
