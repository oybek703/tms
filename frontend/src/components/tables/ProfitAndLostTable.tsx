...
const ProfitAndLostTable: React.FC<ProfitAndLostTableProps> = ({ pickedDate, rows }) => {
  ...
  return (
    <TableContainer component={Paper}>
      ...
      <Table id={`profit-and-lost-${selectedDate}`} size="small"
        ...
        <TableHead className={classes.stickyHead}>
          <TableRow>
            ...
            <TableCell><BoldWithColor>Наименование
              показателей</BoldWithColor></TableCell>
            <TableCell
              align="center"><BoldWithColor>{yearFirstDate}</BoldWithColor></TableCell>
            <TableCell
              align="center"><BoldWithColor>{monthFirstDate}</BoldWithColor></TableCell>
            <TableCell
              align="center"><BoldWithColor>{selectedDate}</BoldWithColor></TableCell>
            <TableCell align="center"><BoldWithColor>Ежемесячная
              разница</BoldWithColor></TableCell>
            <TableCell align="center"><BoldWithColor>Ежемесячная разница
          ...
          {rows.map((row: any, index: number) => (
            <TableRow hover key={index}>
              ...
              <TableCell>{row['isTableHead'] ?
                ...
                row['indicator_name']}</TableCell>
              <TableCell align="center">
                {row['isTableHead'] ?
                  <b>{formatNumber(row['yearBegin'])}</b> :
                  formatNumber(row['yearBegin'])}
              </TableCell>
              <TableCell align="center">
                {row['isTableHead'] ?
                  <b>{formatNumber(row['monthBegin'])}</b> :
                  formatNumber(row['monthBegin'])}
              </TableCell>
              <TableCell align="center">
                {row['isTableHead'] ?
                  <b>{formatNumber(row['selectedDate'])}</b> :
                  formatNumber(row['selectedDate'])}
              </TableCell>
              <TableCell align="center">
                {row['isTableHead'] ?
                  <b>{formatNumber(row['differ'], true)}</b> :
