...
const WarningAlert: React.FC<WarningDataInterface> = ({ rows, role }) => {
  ...
  return (
    <Alert
      type='warning'
      message={`Central Bank Standard is not entered yet. ${role === 'admin' ?
        'Please, make sure it is    entered.' :
        'Report admin user about it.'}`}/>
  )
}
            ...
            <TableBody>
              <TableRow hover>
                ...
                <TableCell align='left'>
                  <b><FormattedData number={Number.isNaN(deviationSum) ? 0 : deviationSum} variant='h6'/></b>
                </TableCell>
                <TableCell align='left'>
                  <b><FormattedData number={Number.isNaN(consumptionSum) ? 0 : consumptionSum} variant='h6'/></b>
                </TableCell>
              </TableRow>
            </TableBody>
          </Table>
