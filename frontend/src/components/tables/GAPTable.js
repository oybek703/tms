    ...
    return (
        <Fragment>
            <TableContainer classes={{root: classes.tableContainer}} component={Paper}>
                <ExportButton id={`gap-${formatOneDate(new Date())}`}/>
                <Table id={`gap-${formatOneDate(new Date())}`} size="small" aria-label="a dense table">
                    <GapTableHead months={months}/>
                    <TableBody>
                        <VerticalColumn data={sourceOfLiquidity} text='источники ликвидности'/>
                        ...
                        <VerticalColumn data={needsOfLiquidity} text='потребности ликвидности'/>
                        <InnerDataRows data={needsOfLiquidity} months={months}/>
                        <TotalOrBoldRow blueBackground months={months} total={needsOfLiquidityTotal}/>
                        {vlaLcrData.map((row, index) => <TotalOrBoldRow withPercent={index === 3} blueBackground={index === 3}
                            key={uuid()} align='left'
                            months={months} total={row}/>)}
                    </TableBody>
                </Table>
            </TableContainer>
            <br/>
