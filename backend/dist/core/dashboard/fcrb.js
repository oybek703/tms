...
class Fcrb extends base_1.Base {
    constructor() {
        super(...arguments);
        this.percentQuery = (depositTypeQuery = '1=1', coaQuery = '1=1') => {
    ...
    async mfi_percents() {
        return await this.getDataInDates(undefined, this.mfiPercentsQuery, true);
    }
    async treasury() {
        return await this.getOneRow(`((BAL LIKE '206%' AND BAL <> '20606') OR (BAL IN ('21022', '21032', '21042', '21010')))`);
    }
    async treasury_percents() {
        return await this.getDataInDates(undefined, this.percentQuery.bind(this, `CON.DEPOSIT_TYPE IN (2, 3, 5)`, `(COA IN ('21022', '21032', '21042') OR (COA LIKE '204%' OR COA LIKE '206%' AND COA NOT IN ('20406', '20606')))`), true);
    }
    async retail() {
        return await this.getOneRow(`BAL IN ('20206', '20406', '20606', '22616', '22617', '22618')`);
    }
    async retail_percents() {
        return await this.getDataInDates(undefined, this.retailPercentsQuery, true);
    }
    async capital() {
    ...
    async other_actives() {
        return await this.getOneRow(`(BAL LIKE '108%'
                        OR BAL LIKE '109%'
                        OR BAL LIKE '115%'
                        OR BAL LIKE '117%'
                        OR BAL LIKE '164%'
    ...
    async bills_and_interbank_deposits() {
        return await this.getOneRow(`BAL IN ('10301', '10501', '10531', '10331', '10321', '10521', '10541', '10341')`);
    }
    async crediting_and_accredetiv() {
        return await this.getOneRow(`BAL IN('11901','12101','12301','12401','12601','12621','12701','12704','12801','12802'
    ...
    async retail_lending() {
        return await this.getOneRow(`BAL IN('12501','12502','12503','12504','12521','14901','14902','14903','14904','14913','14921','12505','14905')`);
    }
    async treasury_portfolio() {
        return await this.getOneRow(`((BAL LIKE '107%') OR BAL LIKE '158%' OR BAL LIKE '159%' OR BAL = '10597')`);
    }
    ...
    async funding_avg_rate_percents() {
        return await this.getDataInDates(undefined, this.fundingAvgRatePercents, true);
    }
    async getRows() {
        const [mfiTotal, mfiPercents, treasuryTotal, treasuryPercents, retailTotal, retailPercents, capital, obligations, otherActives, investments, billsAndInterbankDeposits, creditingAndAccredetiv, retailLending, treasuryPortfolio, balanceActive, fundingAvgRatePercents] = await Promise.all([
            this.mfi(),
            this.mfi_percents(),
            this.treasury(),
