...
class InterbankDeposits extends correspondent_base_1.CorrespondentBase {
    constructor() {
        super(...arguments);
        this.innerBanks = [
    ...
    async accounts_settlement() {
        return await this.getOneRow('-', 'Расчеты с клиентами - 29801', '', super.formatQuery.bind(this, '29801'));
    }
    async customer_funds() {
        return await this.getOneRow('-', 'Cредства клиентов для конвертации - 22613', '', this.customerFundsQuery);
    }
    async getRows(others) {
        const innerBankPromises = this.innerBanks.map(({ name, code }) => this.getOneRow('-', name, code, null));
