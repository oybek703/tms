...
const {returnDiffer} = require('../mainIndicators/mi_pure_functions')
const {
    getMainIndicatorsTotalData,
    createMainIndicatorsData
} = require('../mainIndicators/mi_pure_functions')
...
class ProfitLostMainClass extends MainClass {
    ...
    async getOneRow(count, indicator_name, role = 'O_O',  isTableHead = false) {
    ...
    percent_income (...args)  { /* Процентные доходы */
        return {
            count: '',
    ...
    percent_expenses (...args)  { /* Процентные расходы */
        return {
            count: '',
    ...
    clear_percent_expenses (income, expense)  { /* Чистый процентный доход */
        return {
            count: '',
    ...
    clear_income_after_reserve (clear_income, potential_loss_sum)  { /* Чистый процентный доход после вычета резервов */
        return {
            count: '',
            ...returnDiffer(
    ...
    clear_interest_free_income (if_income, if_expenses)  { /* Чистые беспроцентные доходы */
        return {
            count: '',
            ...returnDiffer(
    ...
    operational_incomes (after_reserve_income, clear_if_income)  { /* Операционные доходы */
        return {
            count: '',
            ...returnDiffer(
                'Операционные доходы',
    ...
    operational_expenses (...args)  { /* Операционные расходы */
        return {
            count: '',
    ...
    operational_result (op_incomes, op_expenses)  { /* Операционные результаты */
        return {
            count: '',
            ...returnDiffer(
    ...
    report_for_period (oper_result, tax_ass)  { /* Чистая прибыль (убыток) на отчетный период */
        return {
            count: '',
            ...returnDiffer(
...
module.exports = ProfitLostMainClass