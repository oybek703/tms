import { createMainIndicatorsData, getMainIndicatorsTotalData, returnDiffer, StringOrNumber } from '../mainIndicators/mi_pure_functions'
...
class ProfitLostMainClass extends MainClass {
    ...
    async getOneRow(count: string, indicator_name: string, role = 'O_O',  isTableHead = false) {
        const {YEAR_BEGIN, MONTH_BEGIN, SELECTED_DATE, DIFFER, DIFFER_PERCENT} = await this.getDataInDates(
            '',
            this.profitLostQuery(role)
        )
        return createMainIndicatorsData(count, indicator_name, YEAR_BEGIN, MONTH_BEGIN,
            SELECTED_DATE, DIFFER, DIFFER_PERCENT, isTableHead)
    }

    async loan_portfolio_interest() { /* Проценты по кредитному портфелю */
    ...
    async investment_interest () { /* Проценты по инвестициям в ценные бумаги */
        return await this.getOneRow(
            '',
    ...
    async interbank_deposits_interest () { /* Проценты по межбанковским депозитам */
        return await this.getOneRow(
            '',
    ...
    async other_interest_income () { /* Другие процентные доходы */
        return await this.getOneRow(
            '',
    ...
    percent_income (...args: any)  { /* Процентные доходы */
        return {
            count: '',
    ...
    async payable_interest () { /* Проценты к оплате по депозитам клиентов */
        return await this.getOneRow(
            '',
    ...
    async borrowing_interest () { /* Проценты к оплате по заимствованиям */
        return await this.getOneRow(
            '',
    ...
    async other_interest_expenses () { /* Другие процентные расходы */
        return await this.getOneRow(
            '',
    ...
    percent_expenses (...args: any)  { /* Процентные расходы */
        return {
            count: '',
    ...
    clear_percent_expenses (income: any, expense: any)  { /* Чистый процентный доход */
        return {
            count: '',
    ...
    async potential_loss_provisions () { /* Резервы возможных убытков (кредиты) */
        return await this.getOneRow(
            '',
    ...
    async potential_loss_provisions_sum () { /* Резервы возможных убытков */
        return await this.getOneRow(
            '',
    ...
    clear_income_after_reserve (clear_income: any, potential_loss_sum: any)  { /* Чистый процентный доход после вычета резервов */
        return {
            count: '',
            ...returnDiffer(
    ...
    async interest_free_income () { /* Беспроцентные доходы */
        return await this.getOneRow(
            '',
    ...
    async interest_free_expenses () { /* Беспроцентные расходы */
        return await this.getOneRow(
            '',
    ...
    clear_interest_free_income (if_income: any, if_expenses: any)  { /* Чистые беспроцентные доходы */
        return {
            count: '',
            ...returnDiffer(
    ...
    operational_incomes (after_reserve_income: any, clear_if_income: any)  { /* Операционные доходы */
        return {
            count: '',
            ...returnDiffer(
                'Операционные доходы',
                clear_if_income,
                after_reserve_income,
                true
            )
        }
    } /* Операционные доходы */

    async salaries_costs () { /* Заработная плата и другие расходы на сотрудников */
        return await this.getOneRow(
            '',
    ...
    async rent_and_maintenance () { /* Аренда и содержание */
        return await this.getOneRow(
            '',
            'Аренда и содержание',
            'R_M'
    ...
    async travel_transport () { /* Командировочные и транспортные расходы */
        return await this.getOneRow(
            '',
    ...
    async administrative_expenses () { /* Административные расходы */
        return await this.getOneRow(
            '',
    ...
    async representation_and_charity () { /* Репрезентация и благотворительность */
        return await this.getOneRow(
            '',
    ...
    async depreciation_costs () { /* Расходы на износ */
        return await this.getOneRow(
            '',
            'Расходы на износ',
            'D_C'
    ...
    async insurance_taxes () { /* Страхование, налоги и другие расходы */
        return await this.getOneRow(
            '',
    ...
    async potential_loss_assessment () { /* Оценка возможных убытков */
        return await this.getOneRow(
            '',
    ...
    operational_expenses (...args: any)  { /* Операционные расходы */
        return {
            count: '',
    ...
    operational_result (op_incomes: any, op_expenses: any)  { /* Операционные результаты */
        return {
            count: '',
            ...returnDiffer(
    ...
    async tax_assessment () { /* Оценка подоходного налога */
        return await this.getOneRow(
            '',
    ...
    report_for_period (oper_result: any, tax_ass: any)  { /* Чистая прибыль (убыток) на отчетный период */
        return {
            count: '',
            ...returnDiffer(
