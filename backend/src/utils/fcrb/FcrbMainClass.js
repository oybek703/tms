...
class FcrbMainClass extends MainClass {
    ...
    formatQuery(date, where_query = '1=1') {
        return `SELECT ABS(ROUND(SUM(SALDO_ACTIVE_EQ + SALDO_PASSIVE_EQ) / POWER(10, 11), 2)) AS SUM
                FROM IBS.SVOD_SALDO_DUMP@IABS
                WHERE ${where_query} AND DAT = TO_DATE('${date}', 'DD.MM.YYYY')`
    }

    percentQuery(date, deposit_type_query = '1=1', coa_query = '1=1') {
    ...
    fundingAvgRatePercents(date) {
        return `SELECT DECODE(CODE_CURRENCY, '000', 'UZS', '840', 'USD', '978', 'EUR', CODE_CURRENCY) CURRENCY_NAME,
                       ROUND((SUM(PERCENT_RATE * SALDO_EQUIVAL_OUT)*DECODE(CODE_CURRENCY, '000',1.11,'840',1.23,'978',1.3,0)) / DECODE(SUM(SALDO_EQUIVAL_OUT), 0, 1, SUM(SALDO_EQUIVAL_OUT)), 2) AS PERCENT
                FROM (SELECT SUBSTR(ACCOUNT_CODE, 13, 3) AS CODE_CURRENCY,
                             ACCOUNT_CODE,
                             PERCENT_RATE,
                             NVL((SELECT /*+index_desc(s UK_SALDO_ACCOUNT_DAY)*/ ABS(
    ...
    async getOneRow(where_query) {
        ...
        const {SUM} = await this.getDataInDates(where_query)
        return SUM
    }

    async mfi() { /* МФИ */
        return await this.getOneRow(
            `(BAL LIKE '220%' OR BAL LIKE '216%')`
        )
    } /*  МФИ */

    async mfi_percents() { /* МФИ Проценты */
        // TODO temporary data
    ...
    async treasury() { /* Казначейство (депозиты юр.лиц + меж.банк) */
        return await this.getOneRow(
            `((BAL LIKE '206%' AND BAL <> '20606') OR (BAL IN ('21022', '21032', '21042', '21010')))`
        )
    } /*  Казначейство (депозиты юр.лиц + меж.банк) */

    async treasury_percents() { /* Казначейство Проценты */
    ...
    async retail() { /* Розница (депозиты физ. лиц) */
        return await this.getOneRow(
            `BAL IN ('20206', '20406', '20606', '22616', '22617', '22618')`
        )
    ...
    async capital() { /* Капитал */
        return await this.getOneRow(
            `(BAL LIKE '3%' OR BAL LIKE '4%' OR BAL LIKE '5%')`
        )
    } /*  Капитал */

    async obligations() { /* ОБЯЗАТЕЛЬСТВА */
        return await this.getOneRow(
            `((BAL LIKE '2%' AND BAL NOT LIKE '222%') OR BAL LIKE '175%')`
        )
    } /*  ОБЯЗАТЕЛЬСТВА */
    ...
    async bills_and_interbank_deposits() { /* цен. бумаги и меж. банк. */
        return await this.getOneRow(
            `BAL IN ('10301', '10501', '10531', '10331', '10321', '10521', '10541', '10341')`
        )
    } /*  цен. бумаги и меж. банк. */

    async crediting_and_accredetiv() { /* кредитования + аккредитив */
        return await this.getOneRow(
    ...
    async funding_avg_rate_percents() { /* *Средняя ставка всего фондирования Проценты */
        // TODO temporary data
        // return await this.getDataInDates(
        //     '',
        //     false,
    ...
    async getRows() {
        ...
        treasuryTotal = obligations - (mfiTotal + retailTotal)
        const treasuryData = {treasuryTotal, treasuryPercents}
        const portfolioData = {
            otherActives, investments, billsAndInterbankDeposits,
            creditingAndAccredetiv, retailLending, treasuryPortfolio,
            balanceActive, fundingAvgRatePercents
        }
