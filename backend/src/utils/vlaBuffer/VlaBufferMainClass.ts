...
class VlaBufferMainClass extends MainClass {
	...
	async getOneRow(whereQuery: string, indicatorName: string, ownQuery: OwnQuery = null) {
		...
		const res = ownQuery
			...
			: await this.getDataInDates(whereQuery)
	...
	income_generating_assets(governBills: VlaBufferCellInterface, overnight: VlaBufferCellInterface) {
	...
	high_liquidity_assets(...args: VlaBufferCellInterface[]) {
	...
	total_demand_liabilities(...args: VlaBufferCellInterface[]) {
