...
class Actives extends MainIndicatorsMainClass {
    ...
    cash_equivs (...args: any) { /* Наличные и денежные эквиваленты */
        return {
            count: '',
    ...
    bills (...args: string[]) { /* Ценные бумаги */
        return {
            count: '',
    ...
    client_credits (...args: any) { /* Выданные кредиты клиентам */
        return {
            count: '',
    ...
    async getRows () {
        ...
        // @ts-ignore
        const clientCredits = this.client_credits(loanPortfolio,
            potentialLossReserves)
        return [
            cashEquivs,
            cashAtCheckOut,
