...
class PlatMainClass extends MainClass {
		...
		return () => {
			return `SELECT SALDO_EQUIVAL_OUT SUM 
              FROM (SELECT * FROM PLACED_ATTRACTED ORDER BY OPER_DAY DESC)
                            WHERE OPER_DAY<TO_DATE('${this.date}', 'DD-MM-YYYY') 
                              AND ROLE='${role}' 
                              AND ROWNUM=1`
		}
	}
	...
	async demand_deposits() {
		...
		return await this.getOneRow('Депозиты до востребования', '202', '', this.platQuery('DD'), true)
	} /* Депозиты до востребования */
	...
	async saving_deposits() {
		...
		return await this.getOneRow('Сберегательные депозиты', '204', '', this.platQuery('SD'))
	} /* Сберегательные депозиты */

	async time_deposits() {
		/* Срочные депозиты */
		return await this.getOneRow('Срочные депозиты', '206', '', this.platQuery('TD'), true)
	} /* Срочные депозиты */
	...
	async other_client_deposits() {
		...
		return await this.getOneRow('Другие депозиты клиентов', '226', '', this.platQuery('OCD'), true)
	} /* Другие депозиты клиентов */
	...
	async interest_charges() {
		...
		return await this.getOneRow('Начисленные проценты', '163, 164', '', this.platQuery('ICH'), true)
	} /* Начисленные проценты */
	...
	async requirements() {
		/* МАЖБУРИЯТЛАР (2) */
		return await this.getOneRow('МАЖБУРИЯТЛАР (2)', '', '', this.platQuery('RQ'))
	} /* МАЖБУРИЯТЛАР (2) */
	...
	async getRows() {
		...
		const placedFundsSum = +placedFunds
			.reduce((acc: number, val: FundData) => (acc += val['sum']), 0)
			.toFixed(2)
