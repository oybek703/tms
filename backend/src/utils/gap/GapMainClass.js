...
const {getGapSubOrDivideByMonth} = require('./gap_pure_functions')
const {getData} = require('../../models/db_apis')
const {getGapTotal} = require('./gap_pure_functions')
...
class GapMainClass extends MainClass {
    ...
    inflowQuery() {
        return `SELECT (LCR.TOTAL - VLA.TOTAL)                         AS TOTAL,
                ...
                FROM (SELECT ROUND((SUM(PERCENT_NAT * NATIONAL_CURRENCY / 100) +
                                    SUM(PERCENT_FOR * NVL(FOREIGIN_CURR_SUMM_EQ, 0) / 100)) / POWER(10, 6), 2) AS TOTAL,
                      ...
                      WHERE ROLE IN ('C_O_D', 'R_F_CBU', 'LEND', 'P_B',
                                     'L_C_A', 'L_C_O', 'P_C_L', 'REPO', 'RECIVE')) LCR
                         INNER JOIN (SELECT TOTAL,
                                            NATIONAL_CURRENCY,
                                            FOREIGN_CURRENCY,
                                            'TEMP_COL' AS TEMP_COL
                                     FROM GAP_ANALYSIS_AUTO
                                     WHERE ROLE = 'vla') VLA ON LCR.TEMP_COL = VLA.TEMP_COL`
    }
    ...
    stableFinancingRequiredAmountQuery() {
        return `SELECT (NSFR.TOTAL + LCR.TOTAL)                         AS TOTAL,
                ...
                FROM (SELECT ROUND(SUM((PERCENT_NAT * NATIONAL_CURRENCY + PERCENT_FOR * FOREIGIN_CURR_SUMM_EQ) / POWER(10, 10)),
                                   2)                                                       AS TOTAL,
                      ...
                      FROM NSFR_BUILDER
                      WHERE ROLE IN ('credits', 'i_d', 'invest', 'c_s', 'o_p_b', 'leasing', 'liquidity')) NSFR,
        ...
        return function () {
            return `SELECT ROUND(
                               SUM((PERCENT_NAT * NATIONAL_CURRENCY 
                                        + PERCENT_FOR * FOREIGIN_CURR_SUMM_EQ) 
                                       / POWER(10, 10)), 2) AS TOTAL,
                           ROUND(SUM(PERCENT_NAT * NATIONAL_CURRENCY / POWER(10, 10)), 2) AS NATIONAL_CURRENCY,
                           ROUND(SUM(PERCENT_FOR * FOREIGN_CURRENCY / POWER(10, 10)), 2) AS FOREIGN_CURRENCY
                    FROM NSFR_BUILDER
                    WHERE ${role}`
    ...
    fromOffBalanceSheets15Query() {
        return `SELECT ROUND((PERCENT_NAT * NATIONAL_CURRENCY + PERCENT_FOR * FOREIGIN_CURR_SUMM_EQ) / POWER(10, 8), 2) AS TOTAL,
    ...
    async getOneRow(role, fromManual = false) {
        if (fromManual) return await this.getDataInDates(false, false, this.manualTableQuery(role), true)
        return await this.getDataInDates(role, true, false, true)
    }

    async getMonths() {
        const data = await this.getDataInDates(false, false, this.monthQuery, true)
    ...
    async stable_funding_available_amount() { /* Доступная сумма стабильного финансирования */
        // TODO
    ...
    async stable_funding_required_amount() {/* Необходимая сумма стабильного финансирования */
        // TODO
        return await this.returnTempData(
    ...
    async getRows() {
        ...
        // LCR START
        const out_flow = await this.outFlow()
        const in_flow = await this.inFlow()
        const rowKeys = Object.keys(out_flow).filter(key => key !== 'INDICATOR_NAME')
        // Чистий отток в последующие 30 дней
        const cleanOutFlow = this.getLcrOrNsfrOneRow(
            'Чистий отток в последующие 30 дней',
            rowKeys.reduce((acc, val) => {
                    acc[val] = out_flow[val] - in_flow[val]
                    return acc
                }, {})
        )
        // Высоколиквидные активы
        const highLiqAssets = this.getLcrOrNsfrOneRow(
            'Высоколиквидные активы',
            (vlaLcrData[0] || [])[0]
        )
        // ПРОГНОЗ LCR
        ...
        const lcrData = [lcrForecast, highLiqAssets, cleanOutFlow, out_flow, in_flow]
