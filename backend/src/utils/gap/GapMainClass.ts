...
class GapMainClass extends MainClass {
    ...
    async getOneRow (role: string, fromManual: boolean = false) {
        if (fromManual) return await this.getDataInDates('',
    ...
    async getRows () {
        ...
        // LCR START
        const out_flow: any = await this.outFlow()
        const in_flow: any = await this.inFlow()
        const rowKeys = Object.keys(out_flow).
