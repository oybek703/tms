...
class GapMainClass extends MainClass {
	...
	async returnTempData(
		INDICATOR_NAME = '__INDICATOR_NAME__',
		TOTAL = 0,
		NATIONAL_CURRENCY = 0,
		FOREIGN_CURRENCY = 0
	) {
	...
	async getOneRow(role: string, fromManual = false) {
		if (fromManual) {
			return await this.getDataInDates('', this.manualTableQuery(role), true)
		}
	...
	async stable_funding_available_amount(): Promise<any> {
	...
	async stable_funding_required_amount(): Promise<any> {
