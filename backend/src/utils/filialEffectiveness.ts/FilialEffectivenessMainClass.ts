...
class FilialEffectivenessMainClass extends MainClass {
	...
	formatQuery(date: string) {
		return `WITH CREDIT_PART AS (
                     ...
                     WHERE OPER_DAY = TO_DATE('${date}', 'DD.MM.YYYY') GROUP BY FILIAL_CODE),
						...
						SELECT CREDIT_PART.FILIAL_CODE                      AS "mfo",
									 ...
									 ROUND(((100 * BENEFIT /
													 (SELECT TO_DATE('${date}', 'DD.MM.YYYY') - TRUNC(TO_DATE('${date}', 'DD.MM.YYYY'), 'YYYY') AS DAY
	...
	async getRows() {
		...
		return await this.getDataInDates('true', null, true)
	}
}

export default FilialEffectivenessMainClass
