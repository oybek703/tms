...
const TimeDepoClients = require('../timeDepoClients/TimeDepoClients')
...
class TimeDepositsMainClass extends MainClass {
        ...
        return `SELECT
                ...
                FROM (SELECT
                          ...
                          (SELECT
                           ...
                           WHERE
                                   TO_DATE(DATE_END, 'DD.MM.YYYY')=TO_DATE('${date}', 'DD.MM.YYYY')
                             AND
                                   FILIAL_NAME=FLS.FILIAL_NAME
                             AND OPER_DAY=(SELECT MAX(OPER_DAY) FROM TIME_DEPO_CLIENTS WHERE OPER_DAY<TO_DATE('${date}', 'DD.MM.YYYY'))) COUNT_IN_DATE,
                          (SELECT
                               TRUNC(SUM(SALDO_EQUIVALENT_OUT/POWER(10, 6)), 1)
                           FROM TIME_DEPO_CLIENTS
                           WHERE
                                   TO_DATE(DATE_END, 'DD.MM.YYYY')=TO_DATE('${date}', 'DD.MM.YYYY')
                             AND
                                   FILIAL_NAME=FLS.FILIAL_NAME
                             AND OPER_DAY=(SELECT MAX(OPER_DAY) FROM TIME_DEPO_CLIENTS WHERE OPER_DAY<TO_DATE('${date}', 'DD.MM.YYYY'))) SUM_IN_DATE,
                          (SELECT
                               COUNT(*)
                           FROM TIME_DEPO_CLIENTS
                           WHERE
                                   TO_CHAR(DATE_END, 'MM.YYYY')=TO_CHAR(TO_DATE('${date}', 'DD.MM.YYYY'), 'MM.YYYY')
                             AND
                                   FILIAL_NAME=FLS.FILIAL_NAME
                             AND OPER_DAY=(SELECT MAX(OPER_DAY) FROM TIME_DEPO_CLIENTS WHERE OPER_DAY<TO_DATE('${date}', 'DD.MM.YYYY'))) COUNT_IN_MONTH,
                          (SELECT
                               TRUNC(SUM(SALDO_EQUIVALENT_OUT)/POWER(10, 6), 1)
                           FROM TIME_DEPO_CLIENTS
                           WHERE
                             ...
                             AND
                                   FILIAL_NAME=FLS.FILIAL_NAME
                             AND OPER_DAY=(SELECT MAX(OPER_DAY) FROM TIME_DEPO_CLIENTS WHERE OPER_DAY<TO_DATE('${date}', 'DD.MM.YYYY'))) SUM_IN_MONTH
                      FROM (SELECT UNIQUE FILIAL_NAME FROM TIME_DEPO_CLIENTS) FLS
                               LEFT JOIN (SELECT
                                              FILIAL_NAME,
                                              ROUND(SUM(SALDO_EQUIVALENT_OUT)/POWER(10, 6), 1) YEAR_BEGIN
                                          FROM (SELECT * FROM TIME_DEPO_CLIENTS ORDER BY OPER_DAY DESC)
                                          WHERE OPER_DAY = (SELECT
                                                                MIN(OPER_DAY) YEAR_BEGIN
                                                            FROM TIME_DEPO_CLIENTS
                               ...
                               LEFT JOIN (SELECT
                                              FILIAL_NAME,
                                              ROUND(SUM(SALDO_EQUIVALENT_OUT)/POWER(10, 6), 1) SELECTED_DATE
                                          FROM (SELECT * FROM TIME_DEPO_CLIENTS ORDER BY OPER_DAY DESC)
    ...
    async getRows(date) {
        ...
        const tableData = await this.getDataInDates(true, true, false, true)
        return {tableData, currentBalance, balanceInMonthBegin}
    }
}

module.exports = TimeDepositsMainClass