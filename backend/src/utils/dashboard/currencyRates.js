...
class CurrencyRates extends DashboardMainClass {
    ...
    getLast90Query(date, currencyCode = '840') {
    ...
    async getRatesByType(token = '', type = 3, date = null) {
        ...
        const filteredData = (arrayData || []).map(r => {
            let returnRow = {}
            this.currencyNames.forEach(c => {
    ...
    async getOtherRates() {
        ...
        const {OPER_DAY} = await this.getDataInDates('', false, this.getBeforeDateQuery.bind(this))
    ...
    async getLast90() {
        const resultData = {}
        await Promise.all(this.currencyNames.map(async ({code, nominal}) => {
    ...
    async getRows() {
        ...
        const cbRate = this.currencyNames.map(row => {
