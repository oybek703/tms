...
class DashboardLiquidity extends DashboardMainClass {
	...
	async liquidityByCategory(liquidityCategory: string, withCategory = false) {
		const res = await this.getDataInDates('', this.dashboardLiquidityQuery(liquidityCategory), true)
	...
	async vlaCurrentState() {
		const [data = {}] = await this.getDataInDates('', this.vlaCurrentStateQuery.bind(this), true)
