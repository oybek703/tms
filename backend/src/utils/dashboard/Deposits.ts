...
class Deposits extends DashboardMainClass {
	...
	async time_deposits() {
		const { OTHERS, UZS } = await this.getDataInDates('', this.timeDepositsQuery)
		return [OTHERS, UZS].map((v) => Number(v || 0).toFixed(2))
	}

	async currency_mfi() {
		const { UZS, USD, EUR, JPY } = await this.getDataInDates('', this.currencyMfiQuery)
		return [UZS, USD, EUR, JPY].map((v) => Number(v || 0).toFixed(2))
	}

	async currency_time_deposits() {
		const { UZS, USD, EUR } = await this.getDataInDates('', this.currencyTimeDepositsQuery)
		return [UZS, USD, EUR].map((v) => Number(v || 0).toFixed(2))
	}

	async interbank_deposits() {
		const { LAND, BORROW } = await this.getDataInDates('', this.interbankDepositsQuery)
		return [LAND, BORROW].map((v) => Number(v || 0).toFixed(2))
	}

	async funding_structure() {
		const { MBD, TD, MFI_SHORT, MFI_LONG } = await this.getDataInDates(
			'',
	...
	async currency_mbd() {
		const { UZS, USD, EUR } = await this.getDataInDates('', this.currencyMBDQuery)
		return [UZS, USD, EUR].map((v) => Number(v || 0).toFixed(2))
	}
