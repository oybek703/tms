const InterBankDepositsMainClass = require("./InterBankDepositsMainClass")
...
class InterbankDeposits extends InterBankDepositsMainClass {
    ...
    formatQuery(date, {currency_code, code_coa}) {
        return `SELECT
                ...
                FROM (SELECT
                               ...
                               FROM (SELECT
                                         NAME_BANK,
                                         SALDO_OUT,
                                         (SELECT DECODE(TO_CHAR(DATE_BEGIN, 'YYYY'), '2099', TO_CHAR(NULL), TO_CHAR(DATE_BEGIN, 'YYYY.MM.DD')) FROM DUAL) BEGIN_DATE,
                                     ...
                                     WHERE OPER_DAY=(SELECT MAX(OPER_DAY) FROM INTERBANKDEPOSIT WHERE OPER_DAY<TO_DATE('${date}', 'DD.MM.YYYY'))
                                       AND ${code_coa} AND CURRENCY_CODE='${currency_code}'
                                     ORDER BY SALDO_OUT DESC)) GROUP BY BANK_CODE`
    }

    async mapBanks(currency_code, code_coa) {
        let allMappedBanks = await this.getDataInDates(
            {currency_code, code_coa},
            true,
            '',
            true
        )
    ...
    async getRows(date) {
        ...
        const land = await Promise.all(currencyCodes.map(currency_code => this.mapBanks(
            currency_code,
            "CODE_COA IN ('10597', '10397', '10521', '10531', '10541', '10321', '10331')"
        )))
        const borrow = await Promise.all(currencyCodes.map(currency_code => this.mapBanks(
            currency_code,
...
module.exports = InterbankDeposits