...
class InterBankDepositsMainClass extends MainClass {
    constructor(date, type = 'borrow') {
        super(date)
        this.type = type
    }

    formatQuery(date, currency_code) {
        return `SELECT
                    NAME_BANK,
                    SALDO_OUT,
                    BEGIN_DATE,
    ...
    formatMappedBanks(allMappedBanks = []) {
        ...
        const percents = allMappedBanks.map(b => +(b['SALDO_OUT']*(b['PERCENT_RATE'])/  mappedBanksSum).toFixed(1))
        const percentSum = +[...percents].reduce((a, b) => a+b, 0).toFixed(1)
        const forDaySum = +[...allMappedBanks].reduce((acc, val) => acc+=val['FOR_DAY'], 0).toFixed(2)
        const forPeriodSum = +[...allMappedBanks].reduce((acc, val) => acc+=val['FOR_PERIOD'], 0).toFixed(2)
        return {allMappedBanks, sumRow: ['Итого', mappedBanksSum, percentSum, forDaySum, forPeriodSum, '100%']}
    }

    async mapBanks(currency_code) {
        let allMappedBanks = await this.getDataInDates(currency_code, true, '', true)
...
module.exports = InterBankDepositsMainClass