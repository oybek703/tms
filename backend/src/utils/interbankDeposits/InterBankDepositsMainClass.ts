...
class InterBankDepositsMainClass extends MainClass {
    ...
    constructor (date: string, type: string = 'borrow') {
        super(date)
        this.type = type
    }
    ...
    formatMappedBanks (allMappedBanks: any = []) {
        const mappedBanksSum = [...allMappedBanks].reduce(
            (acc, val) => acc += val['SALDO_OUT'], 0)
        allMappedBanks = allMappedBanks.map((b: any) => {
            b['PERCENT_SHARE'] = +(b['SALDO_OUT'] * 100 /
                mappedBanksSum).toFixed(1)
            return b
        })
        const percents = allMappedBanks.map(
            (b: any) => +(b['SALDO_OUT'] * (b['PERCENT_RATE']) /
                mappedBanksSum).toFixed(1))
        const percentSum = +[...percents].reduce((a, b) => a + b, 0).toFixed(1)
        const forDaySum = +[...allMappedBanks].reduce(
            (acc, val) => acc += val['FOR_DAY'], 0).toFixed(2)
        const forPeriodSum = +[...allMappedBanks].reduce(
            (acc, val) => acc += val['FOR_PERIOD'], 0).toFixed(2)
        return { allMappedBanks,
            sumRow: [
                'Итого',
                mappedBanksSum,
                percentSum,
    ...
    async mapBanks (currencyCode: string) {
        let allMappedBanks = await this.getDataInDates(currencyCode, null, true)
