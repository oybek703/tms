...
import { corrInclude, getCorrespondentTotal } from './corr_pure_functions'

/* eslint-disable camelcase */
class TotalCash extends CorrespondentMainClass {
	otherBanks: { name: string; code: string }[]
	...
	cash_at_checkout(total: any, on_hand: any) {
	...
	total_correspondent_accounts(nostro: any, ob_nostro: any) {
	...
	async nostro() {
		/* Ностро */
		return await this.getOneRow('-', 'Ностро', '', this.nostroQuery)
	} /* Ностро */

	async f_o_r() {
		/* ФОР */
		return await this.getOneRow('-', 'ФОР', '', this.forQuery)
	} /* ФОР */
