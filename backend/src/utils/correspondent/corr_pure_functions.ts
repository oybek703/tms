export function createCorrespondentData(
    ...
    KZT: number, RUB: number, CHF: number, GBP: number, USD: number, EUR: number, isTableHead: boolean, isNegative: boolean = false) {
    return {
        count, state,
...
export function corr_include(withText = false) {
    return {
        count: '',
