...
class TotalCashCurrent extends TotalCash {
	...
	async nostro() {
		/* Ностро */
		return await this.getOneRow('-', 'Ностро', `CODE_COA = '10301'`, null)
	} /* Ностро */

	async f_o_r() {
		/* ФОР */
		return await this.getOneRow('-', 'ФОР', `CODE_COA = '10309'`, null)
	} /* ФОР */

	async corr_accounts_with_other_banks() {
		return (await this.getDataInDates('', this.corrAccountsWithOtherBanksQuery, true)).map(
			(b: { NAME?: string }) => ({
				...b,
				count: '-',
				state: b['NAME']
