...
const {corr_include} = require("./corr_pure_functions")
...
class TotalCash extends CorrespondentMainClass {
    ...
    formatQuery(date, where_query = '00000444') {
    ...
    cash_at_checkout(total, on_hand) { /* Наличные деньги в кассе */
        const [UZS, CNY, JPY, KZT, RUB, CHF, GBP, USD, EUR] = this.currs.map(c => total[c]-on_hand[c])
        return {
            count: 1.1,
    ...
    total_correspondent_accounts(nostro, ob_nostro) { /* Всего корреспондентские счета */
        const [UZS, CNY, JPY, KZT, RUB, CHF, GBP, USD, EUR] = this.currs.map(c => nostro[c]+ob_nostro[c])
        return {
            count: 2,
    ...
    async getRows() {
        const otherBankPromises = this.other_banks
            .map(({name, code}) => this.getOneRow('-', name, code))
        const [
            totalCashOnHand,
            cashOnRoad,
            nostro,
