...
import { isYearEnd } from '../dateFormatter'
import { createCapitalData, getCapitalTotal } from './capital_pure_functions'
...
class Capital extends MainClass {
  ...
  formatQuery(date: string, whereQuery: string = `code_coa like '303%'`) {
  ...
  reversePurchasedQuery(date: Date) {
  ...
  pastPeriodsQuery(date: Date) {
    return `SELECT 
       decode(sign(sum(saldo_equival_out)), -1, sum(saldo_equival_out), 0) AS SALDO_EQ_OUT
                FROM
                    (SELECT
                         (SELECT --+index_desc (sl UK_SALDO_ACCOUNT_DAY)
                                 saldo_equival_out
  ...
  fullyPaidSharesQuery(date: Date) {
    return `SELECT
  ...
  totalCapitalInvestmentQuery(date: Date) {
    return `SELECT
                    (SELECT sum(saldo_equival_out)
                     FROM (SELECT
                               (SELECT --+index_desc (sl UK_SALDO_ACCOUNT_DAY)
  ...
  currentYearProfitQuery(date: Date) {
  ...
  async getOneRow(firstRow: string, character: string, codeCoa: string, ownQuery?: OwnQuery, isTableHead: boolean = false) {
    if (!Boolean(codeCoa)) {
      const { SALDO_EQ_OUT } = await this.getDataInDates('', ownQuery)
      return createCapitalData(firstRow, character, SALDO_EQ_OUT, isTableHead)
    }
    const { SALDO_EQ_OUT } = await this.getDataInDates(codeCoa)
    return createCapitalData(firstRow, character, SALDO_EQ_OUT, isTableHead)
  }

  ordinary_shares(full_paid: any, rev_purchased: any) {/* Обыкновенные Акции, чистые */
  ...
  retained_earnings(reserves: any, profit: any, los1: any, los2: any) {/* Нераспределенная прибыль (убыток), в том числе капитальные резервы */
    return createCapitalData(
        '1.3.',
  ...
  capital_before_deduction(...args: any) {/* Итого основного капитала уровня I до вычетов  */
    return createCapitalData(
        '',
  ...
  deduction_from_tier1(investment_total: any, investment_for_other_banks: any, initagible_assets: any) {/* Вычеты из суммы основного капитала уровня I */
    return createCapitalData(
        '1.6.',
        'Вычеты из суммы основного капитала уровня I',
        getCapitalTotal(investment_total, investment_for_other_banks, initagible_assets),
        true
    )
  } /* Вычеты из суммы основного капитала уровня I */
  ...
  total_corrected_capital(capital_before_ded: any, ded_from_t1: any) {/* ВСЕГО СКОРРЕКТИРОВАННЫЙ ОСНОВНОЙ КАПИТАЛ УРОВНЯ I */
    return createCapitalData(
        'I',
  ...
  non_cumulative_shares(fully_paid_shares: any, reverse_repurchased_assets: any) {/* Некумулятивные бессрочные привилегированные акции  */
    return createCapitalData(
        '2.1.',
  ...
  capital_added_total_level1(...args: any) {/* ВСЕГО ДОБАВЛЕННЫЙ КАПИТАЛ УРОВНЯ I */
    return createCapitalData(
        'II',
  ...
  total_adjusted_capital_level1(level1: any, level2: any) {/* ВСЕГО СКОРРЕКТИРОВАННЫЙ КАПИТАЛ УРОВНЯ I */
    return createCapitalData(
        'III',
  ...
  total_capital_level2(...args: any) {/* Итого капитала Уровня II */
    return createCapitalData(
        '',
  ...
  corrected_capital_level2(...args: any) {/* СКОРРЕКТИРОВАННЫЙ КАПИТАЛ УРОВНЯ II */
    return createCapitalData(
        'IV',
  ...
  total_regular_capital(...args: any) {/* ВСЕГО РЕГУЛЯТИВНЫЙ КАПИТАЛ */
    return createCapitalData(
        'V',
