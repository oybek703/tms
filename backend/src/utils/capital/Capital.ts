...
class Capital extends MainClass {
	...
	formatQuery(date: string, whereQuery = `code_coa like '303%'`) {
	...
	ordinary_shares(full_paid: any, rev_purchased: any) {
	...
	async los_for_past_periods() {
		...
		return await this.getOneRow('в', 'Убыток за прошлый год', '', this.pastPeriodsQuery)
	} /* Убыток за прошлый год */

	async los_for_current_year() {
		/* Убыток за текущий год  */
		return await this.getOneRow('г', 'Убыток за текущий год', '', this.currentYearQuery)
	} /* Убыток за текущий год  */

	retained_earnings(reserves: any, profit: any, los1: any, los2: any) {
	...
	async devaluation_reserve() {
		...
		return await this.getOneRow('1.5.', 'Резерв на девальвацию', `code_coa='30906'`, null, true)
	} /* Резерв на девальвацию */

	capital_before_deduction(...args: any) {
	...
	deduction_from_tier1(
		investment_total: any,
		investment_for_other_banks: any,
		intangible_assets: any
	) {
	...
	total_corrected_capital(capital_before_ded: any, ded_from_t1: any) {
	...
	non_cumulative_shares(fully_paid_shares: any, reverse_repurchased_assets: any) {
	...
	capital_added_total_level1(...args: any) {
		/* ВСЕГО ДОБАВЛЕННЫЙ КАПИТАЛ УРОВНЯ I */
		return createCapitalData('II', 'ВСЕГО ДОБАВЛЕННЫЙ КАПИТАЛ УРОВНЯ I', getCapitalTotal(...args))
	} /* ВСЕГО ДОБАВЛЕННЫЙ КАПИТАЛ УРОВНЯ I */

	total_adjusted_capital_level1(level1: any, level2: any) {
	...
	total_capital_level2(...args: any) {
		/* Итого капитала Уровня II */
		return createCapitalData('', 'Итого капитала Уровня II', getCapitalTotal(...args), true)
	} /* Итого капитала Уровня II */

	corrected_capital_level2(...args: any) {
	...
	total_regular_capital(...args: any) {
		/* ВСЕГО РЕГУЛЯТИВНЫЙ КАПИТАЛ */
		return createCapitalData('V', 'ВСЕГО РЕГУЛЯТИВНЫЙ КАПИТАЛ', getCapitalTotal(...args), true)
	} /* ВСЕГО РЕГУЛЯТИВНЫЙ КАПИТАЛ */
