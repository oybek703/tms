...
class RiskPartClass extends DashboardMonthlyMainClass {
	...
	creditPortfolioQuery(curr: string) {
		...
		const all = `SELECT OPER_DAY                                                       DAT,
                           ...
                           ROUND(SUM(TOTAL_LOAN) / POWER(10, 6), 2)                    AS SUM
                    FROM CR.VIEW_CREDITS_DWH@RISK
                    WHERE OPER_DAY BETWEEN TO_DATE('${
											this.firstDate
										}', 'dd.mm.yyyy') AND TO_DATE('${this.secondDate}', 'dd.mm.yyyy')
	...
	async credit_portfolio() {
		/* Кредитный портфель */
		const data = await this.getDataInDates('', this.creditPortfolioQuery.bind(this), true)
		return this.createData('1', 'Кредитный портфель', data, true)
	} /* Кредитный портфель */

	async credit_portfolio_nat_curr() {
		/* в том числе в нац.валюте */
		const data = await this.getDataInDates('', this.creditPortfolioQuery.bind(this, '000'), true)
	...
	async reserve() {
		/* Резервы */
		const data = await this.getDataInDates('', this.reserveQuery.bind(this), true)
