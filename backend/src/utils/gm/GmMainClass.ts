...
class GmMainClass extends MainClass {
	...
	async securitySum() {
		return await this.getDataInDates('', this.securitySumQuery.bind(this))
	}
	...
	async accredetiv() {
		...
		const accredetives = (
			await Promise.all(
				['840', '978', '643'].map((c) =>
					this.getDataInDates('', this.accredetivQuery.bind(this, this.date, c))
				)
			)
		).map((v) => v['SALDO_OUT'])
		...
		const { SALDO_OUT: mio = 0 } = await this.getDataInDates('', this.mioQuery)
		return {
			acs: accredetives,
			others: [
				{ loan_name: 'Непокрытый текущий аккредитив (МИО)', currency_name: 'USD', par_value: mio },
				{
					loan_name: 'Обязательства по аккредитивам (11501)',
					currency_name: 'USD',
					par_value: SALDO_OUT
				}
			]
	...
	async collateralSavings() {
		/* Сберегательный счет (залоговые сред.) */
		return await this.getDataInDates('', this.collateralSavingsQuery)
	} /* Сберегательный счет (залоговые сред.) */

	async autoCredit() {
		/* Сберегательный счет (Автокредит) */
		return await this.getDataInDates('', this.autoCreditQuery)
	} /* Сберегательный счет (Автокредит) */
	...
	async getRows() {
		...
		return {
			tableData: [...tableData.slice(0, 7), collateralSavings, autoCredit, ...tableData.slice(7)],
			accredetiv,
			currRates
		}
	}
