...
class CurrencyPositionMainClass extends MainClass {
	...
	async regular_capital() {
		return await this.getDataInDates('', this.regularCapitalQuery.bind(this))
	}
		...
		const tableSumData = [
			{
				...
				sum: longTotal,
				percent: Number((longTotal / REG_CAP) * 100).toFixed(2)
			},
			{
				title: 'Суммарная величина всех коротких валютных позиций (маx - 15%)',
				sum: shortTotal,
				percent: Number((shortTotal / REG_CAP) * 100).toFixed(2)
			},
			{
				title: 'Суммарная величина открытых валютных позиций (маx - 15%)',
				sum: longAndShortTotal,
				percent: Number((longAndShortTotal / REG_CAP) * 100).toFixed(2)
			},
			{ title: 'Регулятивный капитал банка на отчетную дату (в сумах)', sum: REG_CAP, percent: 0 }
		]
		return {
			allRows,
			tableSumData
