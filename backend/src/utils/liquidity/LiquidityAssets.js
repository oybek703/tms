...
FROM   (SELECT (SELECT SUM(saldo_equival_out)
    ...
    localBanksQuery(date) {
        return `SELECT (SELECT ROUND(SUM(SALDO_EQUIVAL_OUT) / POWER(10, 8), 2)
                FROM   (SELECT (SELECT --+index_desc (sl UK_SALDO_ACCOUNT_DAY)
                       ...
                       (SELECT ROUND(SUM(SALDO_EQUIVAL_OUT) / POWER(10, 8), 2)
                        FROM   (SELECT (SELECT --+index_desc (sl UK_SALDO_ACCOUNT_DAY)
                                               NVL(SALDO_EQUIVAL_OUT, 0) / (SELECT EQUIVAL
                                                                            FROM
                                                                                (SELECT
                                                                                     --+index_desc(rate XPK_S_RATE_CUR )
                                                                                     EQUIVAL,
                                                                                     ROWNUM AS N
                                                                                 FROM
                                                                                     IBS.S_RATE_CUR@IABS
    ...
    liquidity_assets_total(...args) { /* ВСЕГО ликвидных активов */
    ...
    high_liq_assets_total(total_assets, f_o_r, repo_tr, vostr_accounts, localBanks) { /* ИТОГО ВЫСОКО ликвидных активов */
        const [total, nat_curr, for_curr, usa_dollar, evro] = this.columns
    ...
    high_liq_assets_share(high_liq_assets_total, total_actives) { /* Доля высоколиквидных активов в % */
...
module.exports = LiquidityAssets