...
import { getLiquidityTotal, liq_include } from './liq_pure_functions'
...
class Obligations extends LiquidityMainClass {
    ...
    legal_entities (total_depos: any, indvdls: any) { /* Юридические лица */
        const [total, nat_curr, for_curr, usa_dollar, evro] =
            ['total', 'nat_curr', 'for_curr', 'usa_dollar', 'evro'].map(
                p => total_depos[p] - indvdls[p])
        return {
            count: '-',
            state: 'Юридические лица',
            total,
            nat_curr,
            for_curr,
            usa_dollar,
    ...
    other_clients (legal_entities: any, state_owned_comp: any, joint_vn: any) { /* Прочие клиенты */
        const [total, nat_curr, for_curr, usa_dollar, evro] = this.columns.map(
            p => legal_entities[p] -
                getLiquidityTotal(p, state_owned_comp, joint_vn))
        return {
            count: '-',
            state: 'Прочие клиенты',
            total,
            nat_curr,
            for_curr,
            usa_dollar,
    ...
    others (
        other_cl_depos: any, accr_cover: any, fd_onpc: any, fd_onconv: any,
        conved_fd: any) { /* Прочее */
        const [total, nat_curr, for_curr, usa_dollar, evro] = this.columns.map(
            p => other_cl_depos[p] -
                getLiquidityTotal(p, accr_cover, fd_onpc, fd_onconv, conved_fd))
        return {
    ...
    total_demand_liabilities (
        demand_depos: any, vostro_accs: any, other_cl_depos: any,
        other_obls: any) { /* ИТОГО обязательств до востребования */
        const [total, nat_curr, for_curr, usa_dollar, evro] = this.columns.map(
            p => getLiquidityTotal(p, demand_depos, vostro_accs, other_cl_depos,
                other_obls))
        return {
            count: 14,
    ...
    async getRows () {
        ...
        const otherClients = this.other_clients(legalEntities,
            stateOwnedCompanies, jointVentures)
