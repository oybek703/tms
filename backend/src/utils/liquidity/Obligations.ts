...
class Obligations extends LiquidityMainClass {
	...
	legal_entities(total_depos: any, indvdls: any) {
		/* Юридические лица */
	...
	other_clients(legal_entities: any, state_owned_comp: any, joint_vn: any) {
		/* Прочие клиенты */
		const [total, nat_curr, for_curr, usa_dollar, evro] = this.columns.map(
			(p) => legal_entities[p] - getLiquidityTotal(p, state_owned_comp, joint_vn)
		)
		return {
			count: '-',
	...
	others(other_cl_depos: any, accr_cover: any, fd_onpc: any, fd_onconv: any, conved_fd: any) {
		/* Прочее */
		const [total, nat_curr, for_curr, usa_dollar, evro] = this.columns.map(
			(p) => other_cl_depos[p] - getLiquidityTotal(p, accr_cover, fd_onpc, fd_onconv, conved_fd)
		)
		return {
			count: '-',
			state: 'Прочее',
	...
	total_demand_liabilities(
		...
		const [total, nat_curr, for_curr, usa_dollar, evro] = this.columns.map((p) =>
			getLiquidityTotal(p, demand_depos, vostro_accs, other_cl_depos, other_obls)
		)
		return {
			count: 14,
