...
export const getLastUpdateTime = asyncMiddleware(async (req: Request, res: Response) => {
    const query = `SELECT TO_CHAR(LOG_DATE, 'DD/MM/YYYY, HH24:MM', 'NLS_DATE_LANGUAGE = RUSSIAN') LAST_UPDATE_TIME
                   FROM (SELECT LOG_DATE FROM USER_SCHEDULER_JOB_LOG WHERE JOB_NAME = 'DASHBOARD_JOB' ORDER BY LOG_DATE DESC)
                   WHERE ROWNUM = 1`
    const { rows = [] } = await getData(query)
    const {LAST_UPDATE_TIME: last_update} = rows[0] as RowsFirstElement || {}
    res.status(200).json({ success: true, last_update })
})
