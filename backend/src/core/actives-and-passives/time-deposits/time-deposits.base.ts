...
import { TimeDepoClientsBase } from '../time-depo-clients/TDC.base'
import { ITimeDepositsData } from './time-deposits.interface'
import { ITimeDepoClientsData } from '../time-depo-clients/TDC.interface'

export class TimeDepositsBase extends Base {
  monthFirstDate = format(new Date(this.date), 'yyyy-MM-01') as unknown as Date
  ...
  private async getCurrencyRate(currencyCode = '840', date?: Date) {
    const { currencyRate } = await this.oracleService.executeQuery<{
      currencyRate: number
    }>(`SELECT equival as "currencyRate" FROM ibs.s_rate_cur@iabs
                       WHERE date_cross<= DATE '${date || this.date}'-1 AND
                             code='${currencyCode}' and ROWNUM=1 ORDER BY DATE_CROSS DESC`)
    return currencyRate
  }

  private async getSumByCurrency(array: ITimeDepoClientsData[] = [], date?: Date) {
  ...
  async getRows() {
    const TdcInDate = await new TimeDepoClientsBase(this.date, this.oracleService).getRows()
    const TdcInMonthBegin = await new TimeDepoClientsBase(
      this.monthFirstDate,
      this.oracleService
    ).getRows()
    ...
    const tableData = await this.getDataInDates<ITimeDepositsData, true>('1=1', undefined, true)
